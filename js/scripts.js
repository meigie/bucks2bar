...
document.getElementById('username').addEventListener('input', (event) => {
    ...
    event.target.style.border = regex.test(username) ? '2px solid green' : '2px solid red';
});
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    chartTab.addEventListener('click', () => {
        ...
        const getData = (type) => months.map(month => parseFloat(document.getElementById(`${month}-${type}`).value) || 0);
